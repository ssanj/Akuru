...
trait SetFuncs { this:Funcs =>
  ...
  def set(value:MongoObject*): MongoUpdateObject =  toMongoUpdateObject($funcMongo(functionName, combine(value:_*)))
  ...
  private def toMongoUpdateObject(mo: => MongoObject): MongoUpdateObject = MongoUpdateObject(mo)
}