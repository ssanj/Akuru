...
trait AkuruFunctions {
   ...
   */
  type WorkUnit[T <: DomainObject, R] = ConnectionProvider => Either[String, R]
  ...
  case class Executor[T <: DomainObject, R](wu: WorkUnit[T, R]) {
    ...
    def map[S](f: R => S): Executor[T, S] = Executor[T, S](cp => wu(cp).fold(l => Left(l), r => Right(f(r))))

    def flatMap[U <: DomainObject, S](f: R => Executor[U, S]): Executor[U, S] = Executor[U, S](cp => wu(cp).fold(l => Left(l),
