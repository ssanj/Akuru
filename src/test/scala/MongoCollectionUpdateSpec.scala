...
final class MongoCollectionUpdateSpec extends CommonSpec {
  ...
  "A MongoCollection with Updates" should "update an existing value" in {
    ( onTestDB ~~>
            ...
            update[Blog](titleField("Blog updates")) { set(titleField("Blog Updatees") and labelsField(Seq("bl%%g")).splat) } ~~>
            ...
            update[Book](publisherField("artima") and printVersionField(2) and priceField(54.95D))
                      { set(nameField("PISC") and printVersionField(3) and priceField(99.99D))} ~~>
