...
final class MongoCollectionUpsertSpec extends AkuruDSL with CommonSpec {
  ...
  "MongoCollection with Upserts" should "insert a Domain Object that does not exist" in {
    ( initTask ~~>
        ( find many Task where (ownerField === "Bugs Bunny") withResults (t => error("Found a Task")) withoutResults (success) ) ~~>
        ( upsert one Task where (ownerField === "Bugs Bunny") withValues (task) returnErrors ) ~~>
        ( find many Task where (ownerField === "Bugs Bunny") withResults {tasks =>
  ...
  it should "only update a Domain Object that exist" in {
    ( initTask ~~>
        ...
        ( upsert one Task where (ownerField === "Bugs Bunny") withValues ($set(ownerField === "Elmer Fudd" & priorityField === 1)) returnErrors ) ~~>
        ( find many Task where (ownerField === "Bugs Bunny") withResults (t => error("Found Bugs not Elmer")) withoutResults (success) ) ~~>
        ( find many Task where (ownerField === "Elmer Fudd") withResults {tasks =>
          tasks.size should equal (1)
          val t = tasks(0)
