...
final class MongoObjectwithMergeDupesSpec extends CommonSpec {
  ...
  it should "merge dupes and non-dupes" in {
    val mo1:MongoObject = mongo.putMongo("$sort", mongo.putPrimitiveObject("key1", "blah")).putPrimitiveObject("key3", "blue").putPrimitiveObject("key4", "boo")
    val mo2:MongoObject = mongo.putPrimitiveObject("key5", "test").putMongo("$sort", mongo.putPrimitiveObject("key2", "bleee")).putPrimitiveObject("key6", "done")
    val mo3 = mo1.mergeDupes(mo2)
